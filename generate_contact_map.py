...
def main(args):
    ...
    for file in files:
        pid_list.append(file.strip().split('.')[0].split('-')[1])

    for pid in tqdm(pid_list):
        print(pid)
        contact_map, sequence = calc_contact_map(args.input_path + 'AF-' + pid + '-F1-' + 'model_v4', 'A')
        np.save(args.output_path + f'{pid}.npy', contact_map)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_path', type=str,  default='./case_study/contact_map/pdb_file/', help='the directory for storing PDB structure files ')
    parser.add_argument('--output_path', type=str, default='./case_study/contact_map/', help='the directory for saving contact map files')
