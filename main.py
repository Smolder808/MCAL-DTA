...
import torch
import warnings
from utils import DTADataset, custom_collate, load_pickle, proGraph
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.0003)                 # 0.0003 for davis; 0.0001 for kiba
    parser.add_argument('--fold', type=int, help='Fold of 5-CV', default=5)
    parser.add_argument('--running_set', type=str, help='warm; new_drug', default='warm')
    ...
    print(device)
    print("load drug substructure graph and target graph ...")
    drug_graphs_dict = load_pickle(f'data/{dataset}/sub_mol_data.pkl')

    target_graphs_dict = load_pickle(f'data/{dataset}/pro_data.pkl')
    ...
    pro_graph = proGraph(target_graphs_dict, protvec_dict, TRAIN_BATCH_SIZE, device)               
    pro_num = len(pro_graph[-1])
    ...
    for fold_i in range(kfold):  
        ...
        train_dataset = DTADataset(smile_list=train_smiles, seq_list=train_seq, label_list=train_label, mol_data=drug_graphs_dict, pro_data=target_graphs_dict, drug_pretrain_dict=mol2vec_dict)
        train_loader = DataLoader(train_dataset, batch_size=TRAIN_BATCH_SIZE, shuffle=True, collate_fn=custom_collate,num_workers=4)
        valid_dataset = DTADataset(smile_list=valid_smiles, seq_list=valid_seq, label_list=valid_label, mol_data=drug_graphs_dict, pro_data=target_graphs_dict, drug_pretrain_dict=mol2vec_dict)
        valid_loader = DataLoader(valid_dataset, batch_size=TRAIN_BATCH_SIZE, shuffle=True, collate_fn=custom_collate,num_workers=4)
        test_dataset = DTADataset(smile_list=test_smiles, seq_list=test_seq, label_list=test_label, mol_data=drug_graphs_dict, pro_data=target_graphs_dict, drug_pretrain_dict=mol2vec_dict)
        ...
        for epoch in range(1, NUM_EPOCHS + 1):
            ...
            for batch_idx, batch_data in enumerate(train_loader):
                ...
                predictions = model(drug_data, pro_graph)
            ...
            # valid
            mse, rmse, ci, r2, pearson, spearman = test(model, valid_loader, pro_graph, device, is_valid=True)
        ...
        mse, rmse, ci, r2, pearson, spearman = test(predModel, test_loader, pro_graph, device, is_valid=False)
